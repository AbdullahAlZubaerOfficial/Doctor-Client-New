...
const BookAnAppointment = () => {
    ...
    useEffect(() => {
        axios.get(`https://doctor-server-green.vercel.app/menu/${id}`)
            .then((res) => {
                setDoctor(res.data);
            })
            .catch((err) => {
...
const onSubmit = async(data) => {
    ...
    try {
        ...
        const bookAppointmentRes = await axiosPublic.post('/appointments', bookAppointmentItem);
        ...
        icon: "error",
        title: "Error",
        html: `<span style="color: red; font-size: 18px;">${error.response?.data?.error || "You are not Logged In 😥"}</span>`,
                    ...
                    >
                        <div className="p-6">
                            ...
                            <div className="mt-6">
                                ...
                                <div className="mb-4">
                                    <h4 className="font-semibold text-gray-700 mb-2">Specializations:</h4>
                                    <div className="flex flex-wrap gap-2">
                                        {["Dentistry", "Cosmetic", "Implants", "Orthodontics"].map((spec, i) => (
                                            <span 
                                                key={i}
                        ...
                        <form onSubmit={handleSubmit(onSubmit)}>
                            ...
                            >
                                <motion.button
                                    ...
                                    className={`w-full py-3 px-6 rounded-lg text-white font-medium transition-colors ${isSubmitting ? 'bg-blue-400' : 'bg-blue-600 hover:bg-blue-700'}`}
                                >
                                    {isSubmitting ? (
